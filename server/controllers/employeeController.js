...
const updateUser = async (req, res) => {
  ...
  try {
    const user = await Employee.findByIdAndUpdate(_id, req.body, { new: true });
    if (user) {
      res.json(user);
    } else {
      res.status(404).json({ error: "User not found" });
...
const logoutUser = async (req, res) => {
  ...
  const user = await Employee.findOne({ email: res.locals.user.email });
  user.refToken = null;
  user.save();
...
const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    const userAvailable = await Employee.findOne({
      email,
    });
    if (userAvailable) {
      res.status(400).json({
